...
class ChatRepository extends BaseRepository {
  ...
  CollectionReference getChatRoomMessages(String chatRoomId) {
    return _chatRooms.doc(chatRoomId).collection("message");
  }
  ...
  Future<void> blockUser(String currentUserId, String blockUserId) async {
    ...
    await userRef.update({
      "blockUsers": FieldValue.arrayUnion([blockUserId]),
    });
  }
  ...
  Future<void> unblockUser(String currentUserId, String blockUserId) async {
    final userRef = firebaseFirestore.collection("users").doc(currentUserId);
    await userRef.update({
      "blockUsers": FieldValue.arrayRemove([blockUserId]),
    });
  }
  ...
  Stream<bool> isUserBlocked(String currentUserId, String otherUserId) {
    return firebaseFirestore
        ...
        .map((doc) {
          ...
          return userData.blockUsers.contains(otherUserId);
        });
  }
  ...
  Stream<bool> amIBlocked(String currentUserId, String otherUserId) {
    return firebaseFirestore
        ...
        .map((doc) {
          final userData = UserModel.fromFirestore(doc);
          return userData.blockUsers.contains(currentUserId);
        });
  }
}
